...
## 综述
OlivOS 中插件运作的模式为，OlivOS 在收到消息时生成事件，事件会按照已加载的插件的优先级以此让事件依次通过事件入口函数进入各个插件的处理流程，插件在处理过程中可以按照自己的业务逻辑进行判断，并在合适的时机主动调用插件事件附带的方法，或是插件加载器进程附带的方法，以实现诸如回复消息一类的业务逻辑，完成后返回
